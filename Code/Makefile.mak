...
program_INCLUDE_DIRS := . / /usr/local/cuda/include/ /usr/src/NVIDIA/cuda-samples/Common/ /usr/local/cuda/include/thrust/ #C++ Include directories
program_CU_INCLUDE_DIRS := ./cuda_util/  /home/users/amclaugh/CUB/cub-1.3.2/ /usr/src/NVIDIA/cuda-samples/Common/ /usr/local/cuda/include/ /usr/local/cuda/include/thrust/ #CUDA Include directories

# Compiler flags
CPPFLAGS += -w $(foreach includedir,$(program_INCLUDE_DIRS),-I$(includedir))#-Xcompiler -openmp
CXXFLAGS += -g -O3 -std=c++0x -Wall -pedantic  

# linker flags
LDFLAGS := -fopenmp

GEN_SM35 := -gencode=arch=compute_61,code=\"sm_61,compute_61\" #Target CC 3.5, for example
NVFLAGS := -O3 -rdc=true -lcublas -lcusparse #rdc=true needed for separable compilation
NVFLAGS += $(GEN_SM35)
